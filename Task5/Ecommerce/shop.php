    ...
    if(isset($_GET['subcat'])){
        if(is_numeric($_GET['subcat'])){
            ...
            if($findSubcatResult->num_rows == 1){
                $productsResult = $productObject->all("WHERE `products`.`status` = 1 AND `products`.`sub_category_id` = {$_GET['subcat']}");
            }else{
        ...
        if(is_numeric($_GET['cat'])){
            ...
            if($findCatResult->num_rows == 1){
                $productsResult = $productObject->all("WHERE `products`.`status` = 1 AND `sub_categories`.`category_id` = {$_GET['cat']}");
            }else{
                header('location:errors/404.php');die;
            }
        }else{
        ...
        if(is_numeric($_GET['brand'])){
            ...
            $findBrandResult = $brandsObject->getBrandById();
